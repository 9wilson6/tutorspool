						...
						if($db->num_rows == 0){
							...
							if ($results==1) {
								...
								if ($user_type==1) {
									...
									header("Refresh:0; url=tutor-login.php");
								}
			...
			if (password_verify($password, $results->password)) {
				...
				if ($user_type==1) {
					if ($results->status==1) {
						...
						<script>
							window.location.assign("student/createpost.php");
						</script>
					...
					<?php }else{
						...
						if ($results->verified==1) {
							...
							header("location:tutor/not_active.php");
						}
					// }else{
					// 	$error="Sorry your account is under suspension. <br> contact admin for further details";
					// }
			...
			<li class="list-group-item text-center "><a href="<?php echo $zipname ?>" class="text-success">Download as a zip</a><i
				...
				foreach ($files as  $file) {
					...
					<li class="list-group-item">
						<?php echo "<a href='../download.php?file=".urlencode($file)."&dir=".$dir_."'>".$file."</a><i class='fa fa-download ml-3 icon-r'></i><br/>";	 ?>
					</li>
	...
	if (!file_exists($dir)) {
		...
		if(empty($files)) {
			...
			<li class="list-group-item text-center ">
				<a href="<?php echo $zipname ?>" class="text-success">Download as a zip</a><i
				class="far fa-file-archive icon-r ml-3"></i></li>
				<?php $zip->close();
				$files=array_diff($allFiles, array('.', '..', "Order_". $project_id."_.zip"));
				foreach ($files as  $file) {
	...
	if (!file_exists($dir)) {
		...
		if(empty($files)) {
			...
			foreach ($files as  $file) {
				?>
				<li class="list-group-item">
					<?php echo "<a href='../delete_file.php?id={$project_id}&&file=".urlencode($file)."&dir=".$dir_."'>".$file." <i class='far fa-trash-alt text-danger ml-3'></i></a><br/>";	 ?>
				</li>
...
function sendMail($details,$to, $subject, $reset=0){
	...
	$mail->SMTPAuth=true;
		...
		$mail->SMTPdebug=2;
		$mail->isHTML(true);
		$mail->Username="admin@tutorspool.com";
		$mail->Password="admin@tutorspool.com";
		$mail->setFrom("admin@tutorspool.com", "Tutorspool");
		$mail->addAddress($to);
 //or 465 if ssl
		...
		<img style="height: 30px; width:30px; position: relative; bottom: -10px; right: -3px;" src="https://www.Tutorspool.com/images/icon.png">
		<span style="color: #f1c40f;">Tutorspool</span></div>
		<div style="font-size: 10px; line-height: 25px; color:#000;"> <br>'. $details .'<br>
		<small style="font-size:13px;"><a href="https://www.Tutorspool.com" target="blank">For more details follow this link....</a></small>
		<br>
		<small>Date: '.date("Y-m-d H:i:s").'</small>
		</div>
		<div style="background: #27ae60; margin: 15px auto; color: #f1c40f; font-size: 14px; padding-top: 20px; border-radius: 10px; text-align: center; min-height: 40px;">Copyright © '.date("Y").' <a style="text-decoration:none; color: #f1c40f;" href="https://www.Tutorspool.com"> Tutorspool.com </a>,All rights reserved.</div>
		</div>

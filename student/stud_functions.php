...
function create_post(){
  if (isset($_POST['submit'])) {
    ...
    global $db, $success, $error, $date_global;
// echo date("Y-m-d H:i:sa", strtotime($date_global));
  ...
  // ,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,//
    if($_FILES['file']['name'][0]=="") {
      ...
      header("Refresh:1; url=my-homework.php");
    }else{
   ...
   // $success="Homework Posted Successfully";
    ...
    sendMail($details, "admin@tutorspool.com", $title);
  }else{
...
// echo date("Y-m-d H:i:sa", strtotime($date_global));
    ...
    if ($date==0 && $time==0) {
      $error="Invalide deadline";
      header("location:homework_edit.php?id=".$project_id."&error=".$error);
    }
    if ($pages==0 && $slides==0 && $problems==0) {
      $error="pages/slides/problems cannot all be zero";
      header("location:homework_edit.php?id=".$project_id."&error=".$error);
   ...
   if ($db->query($query)) {
    ...
    header("location:my-homework-details.php?id=".urlencode(convert_uuencode($project_id)));
  }else{
    $error="Was not Successfully Posted try again";
    header("location:homework_edit.php?id=".urlencode(convert_uuencode($project_id))."&error=".$error);
  }
}
}
